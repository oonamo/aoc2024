...
int is_xmas(Matrix *, unsigned long r, unsigned long c);
...
int solve(char *file)
{
    ...
    for (unsigned long i = 0; i < mat->rows; i++)
    {
        ...
        {
            if (mat->items[i][j] != XMAS[0])
                continue;
            result += is_xmas(mat, i, j);
        }
    }
...
int is_xmas(Matrix *m, unsigned long r, unsigned long c)
{
    bool rdw = right_diagnoal_wise(m, r, c);
    bool rdw_rev = right_diagnoal_reverse(m, r, c);
    bool ldw = left_diagnoal_wise(m, r, c);
    bool ldw_rev = left_diagnoal_reverse(m, r, c);

    int hits = 0;
    if (rdw && left_diagnoal_wise(m, r, c + 2))
    {
        hits++;
    }

    if (rdw && right_diagnoal_reverse(m, r + 2, c))
    {
        hits++;
    }

    if (ldw && left_diagnoal_reverse(m, r + 2, c))
    {
        hits++;
    }

    if (rdw_rev && left_diagnoal_reverse(m, r, c + 2))
    {
        hits++;
    }
